...
perform_analysis <- function(datasets, projection_methods = "PCA",
                             ...
                             cluster_indices = c("cluster_accuracy", "Silhouette_index", "Dunn_index", 
...
generate_projections <- function(data, projection_methods, seed = 42) {
  ...
  projections <- pbmcapply::pbmclapply(projection_methods, function(projecion_method) {
    ...
    tryCatch({
      projection <- performProjection(X = data_clean, projecion_method = projecion_method, Target = target, labels = labels)
...
performProjection <-
  function(X, projecion_method = "PCA", scaleX = TRUE, Target = NULL, switchDims = FALSE, labels = NULL) {
    ...
    valid_projection_methods <- c("none", "PCA", "ICA", "MDS", "tSNE", "isomap", "PLSDA", "IPCA", "Umap") 
...
process_cluster_scores <- 
  function(cluster_scores_df, selected_metrics = c("cluster_accuracy", "Silhouette_index", "Dunn_index", 
                                                   "Rand_index", "DaviesBouldin_index", "dbcv_index")) {
